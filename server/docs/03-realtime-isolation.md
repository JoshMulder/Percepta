...
- **`radio.transmit` exists in the capability list from day one but is never granted.** The
  ...
  the certified radio arrives, the permission model and the certification check
  (DroneOps' `UserCertification` with computed expiry) are already built and tested.
...
station hears the result — so `radio.control` is separate from `radio.listen`: being able to
change what everyone hears is a bigger grant than being able to listen.

It is deliberately **not** exclusive. An earlier version of this document specified a Redis
lease with a timeout and a visible holder. That was dropped on 2026-07-28 as
over-engineering for how the radio is actually used: a station sits on one frequency almost
all of the time, so contention is rare and the ceremony would cost more than it saves.
Anyone holding `radio.control` may tune at will, and the audit log is what answers "who
moved it". `radio.transmit` is the one case that will genuinely need exclusivity — two
transmitters keying one channel is not merely awkward — so revisit when that hardware
exists.
